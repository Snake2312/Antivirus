...
{
	std::u16string filePath = u"Monitors.odd";
...
void Server::saveMonitors()
{
	...
	writer.writeU16String(u"Ohvatov");
...
{
	std::u16string filePath = u"Scanners.odd";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;
...
void Server::startUp()
{
	base = std::shared_ptr<Base>(BaseLoader::load(u"Base.odd"));
	threats = std::make_shared<ThreatList>(u"Threats.odd");
	threats->load();
